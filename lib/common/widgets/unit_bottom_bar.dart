...
class _UnitBottomBarState extends State<UnitBottomBar>
  ...
  late GifController controllers;
  int first = 0;
  ...
  Widget _buildChilds(BuildContext context, int i, int p, Color color) {
    final bool active = p == _position;
    final bool left = i == 0;
    final int bar2 = 0; //BlocProvider.of<GlobalBloc>(context).state.bar2;
    final int bar3 = 0; //BlocProvider.of<GlobalBloc>(context).state.bar3;
    return GestureDetector(
      onTap: () {
        _tapTab(p);
      },
